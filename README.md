...
- `/管理 永封审查 [用户] [原因] [附件(可选)] [审查天数(可选，默认7)]` - 启动永封审查流程，流程结束后自动永封。
- `/管理 永封 [用户] [原因(可选)] [截图(可选)]` - 永久封禁用户。
- `/管理 撤销处罚 [处罚ID] [原因(可选)]` - 撤销指定的处罚记录。
- `/管理 撤销永封审查 [审查ID] [原因] [附件(可选)]` - 按审查ID撤销一个正在进行的永封审查。
...
- `/管理 子区管理 置顶 [子区(可选)]` - 将指定论坛帖子设为置顶帖。
- `/管理 子区管理 取消置顶 [子区(可选)]` - 取消指定论坛帖子的置顶状态。
- `/管理 子区管理 删除 [帖子]` - 删除指定帖子。
- `/管理 答题处罚 [用户]` - 对用户进行答题处罚（通常是移除特定身份组并赋予"答题"身份组）。

### 📌 置顶巡检 (`/置顶巡检`)
- `/置顶巡检 操作:添加 [帖子]` - 将单个论坛帖子加入长期置顶巡检列表。
- `/置顶巡检 操作:批量添加 [帖子列表]` - 通过多个帖子链接/ID 一次性批量加入巡检列表。
- `/置顶巡检 操作:扫描本服当前置顶帖` - 扫描服务器所有论坛中的当前置顶帖，列出链接并支持一键导入。
- `/置顶巡检 操作:列表` - 分页查看当前巡检列表、最近巡检状态与帖子链接。
- `/置顶巡检 操作:立即检查` - 立即执行一次置顶状态巡检并尝试自动恢复掉置顶的帖子。
...
│   ├── thread_pin_watch/       # 帖子置顶巡检模块
│   ├── utils/                  # 工具函数和类
│   └── verify/                 # 验证系统模块
├── data/                       # 数据存储目录 (自动创建)
│   ├── anonymous_feedback.db   # 匿名反馈数据库
│   └── thread_pin_watch/       # 帖子置顶巡检数据
└── logs/                       # 日志文件目录 (自动创建)
```
...
```json
{
    ...
    "cogs": {
        ...
        "thread_pin_watch": {"enabled": true, "description": "帖子置顶巡检功能"}
    },

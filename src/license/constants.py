...
CC_LICENSES = {
    ...
    "CC BY-NC-SA 4.0": {
        ...
        "elements": ["BY", "NC", "SA"],
        "reproduce": "允许转载，但必须保留署名、禁止商用，且转载时必须也采用本协议(CC BY-NC-SA 4.0)进行分享。",
        "derive": "允许二创，但必须保留署名、禁止商用，且二创作品必须也采用本协议(CC BY-NC-SA 4.0)进行分享。",
        "commercial": "禁止",
    ...
    "CC BY-SA 4.0": {
        ...
        "elements": ["BY", "SA"],
        "reproduce": "允许转载，但必须保留署名并以相同方式共享(CC BY-SA 4.0)。",
        "derive": "允许二创，但必须保留署名并以相同方式共享(CC BY-SA 4.0)。",
        "commercial": "允许，但必须保留署名并以相同方式共享(CC BY-SA 4.0)。",
...
HUB_VIEW_CONTENT = (
    ...
    "> - “**禁止**用于头像或做成表情包”\n"
    "> - “**禁止**被收录进任何合集包”\n"
    "> 那么，**你必须使用【自定义】**。这是唯一能精确表达你意愿的方式，在社区内有最好的“君子协定”效果。\n\n"

    "**选项二：【📜 使用CC协议模板】**\n"
    "> ⚠️ 严重警告：选择CC协议，意味着你接受你的作品被无限制地传播到**任何地方**！\n"
    "> 这**必然**会导致你的作品出现在你视野之外的网站、视频、甚至被打包成**免费素材合集**。只要对方遵守了署名、非商用等基本规则，这就是完全合规的。\n"
    "> 如果你**不能接受**上述后果，**请不要使用CC协议**。\n"
    "> 如果你能接受，并希望采用一套国际通用的标准，那么可以选择此项。\n\n"
